...
QUnit.test("testGenerateSecretKey", function (assert) {
    ...
    //Act
    var masterKey = mpw.mpw_core_calculate_master_key(masterPassword, masterKeySalt, function (i) {
        console.log(i);
    });
...
QUnit.test("testPassConvertToPasswordLong", function (assert) {
    //Arrange
    var util = new Util();
    var mpw = new MPW();
...
QUnit.test("testPassConvertToPasswordMaximum", function (assert) {
    //Arrange
    var util = new Util();
    var mpw = new MPW();

    var sitePasswordSeed = util.convertBufferFromHex("c20b97cd72c9c6b19530a46ae86ea48e78d3d279404540cf2d56568a09cbc1a3");
    var siteTypeString = "maximum";

    //Act
    var password = mpw.mpw_core_convert_to_password(siteTypeString, sitePasswordSeed);

    //Assert
    assert.equal(password, "C1$p52dawNfJkN(w^%x#");
});
